...
def fetch_userID(user_mail):
    ...
    db_data_users = fetch_data('cpr_users', email=user_mail)
    try:
...
def displayBalance(user_mail):
    ...
    if userId:
        db_data_accounts = fetch_data('cpr_accounts', user_id=userId)
...
def depositMoney(user_mail,depositAmount):
    ...
    if userId:
        try:
            ...
            update_data('cpr_accounts', values={'balance': balance}, conditions={'user_id': userId})
...
def transferMoney(user_mail,transferAmount):
    ...
    senderAccountDetails = fetch_data('cpr_accounts', user_id=senderUserId)
    ...
    if senderBalance > transferAmount:
        ...
        if receiverAccountId in fetch_column_data('cpr_accounts', 'account_id'):
            ...
            update_data('cpr_accounts', values={'balance': UptSenderBalance}, conditions={'user_id': senderUserId})
            displayBalance(user_mail)

            # fetching receiver account info from flm_accounts
            receiver_db_data = fetch_data('cpr_accounts', account_id=receiverAccountId)
            ...
            update_data('cpr_accounts', values={'balance': receiverBalance}, conditions={'account_id': receiverAccountId})



            # inserting transaction details into flm_transactions table (sender side)
            insert_data('cpr_transactions', user_id=senderUserId, account_id=senderAccountId, amount=transferAmount, from_account=senderAccountNumber,
                        to_account=receiverAccountNumber, trans_type='db')


            # inserting transaction details into flm_transactions table (sender side)
            insert_data('cpr_transactions', user_id=receiverUserId, account_id=receiverAccountId, amount=transferAmount,
...
def transactionHistory(user_email):
    try:
        ...
        query = """
                SELECT cpr_users.first_name, cpr_users.last_name, cpr_accounts.account_number, cpr_transactions.amount, cpr_transactions.from_account, cpr_transactions.to_account, cpr_transactions.trans_date, cpr_transactions.trans_type
                FROM cpr_users
                INNER JOIN cpr_transactions ON cpr_users.user_id = cpr_transactions.user_id
                INNER JOIN cpr_accounts ON cpr_transactions.account_id = cpr_accounts.account_id
                WHERE cpr_users.email = %s
            """
