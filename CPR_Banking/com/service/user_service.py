...
import time
from CPR_Banking.com.service.validations import *
from CPR_Banking.com.service.mail_operations import *
from CPR_Banking.com.service.user_security import *
...
class CreateAccount:
    def accountCreation(self):
        ...
        insert_data('cpr_users', first_name=self.fname, last_name=self.lname, email=self.email, dob=self.dob,
                    salt=self.salt, password=self.password)


        # fetching userId of the new user created
        self.userId = fetch_data('cpr_users', email=self.email)

        # inserting account info into database, table flm_accounts for new user created
        insert_data('cpr_accounts', user_id=self.userId[0][0], account_number=self.accountNumber, balance=self.initialDeposit, is_active=True)
...
def login(user_mail):
    ...
    while True:
        ...
        if user_mail in fetch_column_data('cpr_users', 'email'):
            user_password = input("Password: ")
            print()

            # fetching user data from db using user_mail
            db_data = fetch_data('cpr_users', email=user_mail)
            ...
            if failedLoginAttemps > MAX_LOGIN_ATTEMPTS:
                ...
                if hashed_user_password == db_password:
                    ...
                    update_data('cpr_users', values={'failedcountattempts': 0}, conditions={'email': user_mail})
                    ...
                    update_data('cpr_users', values={'failedcountattempts': failedLoginAttemps}, conditions={'email': user_mail})
                    return False
    ...
    try:
        ...
        if user_dob == str(db_dob):
            ...
            if mailOtpVerification(user_mail):
                ...
                update_data('cpr_users', values={'password': updated_password,'salt': updated_salt}, conditions={'email': user_mail})
